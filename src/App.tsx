...
export default function App() {
  ...
  useEffect(() => {
    if (user) {
      localStorage.setItem("enjez_user", JSON.stringify(user));
    }
  }, [user]);

  const checkIfNeedsSetup = (userData: UserProfile) => {
  ...
  useEffect(() => {
    if (user && (page === "landing" || page === "auth")) {
      setPage(checkIfNeedsSetup(user) ? "setup" : "dashboard");
    }
  }, [user, page]);
  ...
  return (
    ...
    >
      <AnimatePresence mode="wait">
        ...
        {page === "auth" && (
          <AuthPage
            ...
            onComplete={(data) => {
              setUser(data);
              setPage(checkIfNeedsSetup(data) ? "setup" : "dashboard");
            }}
            t={t}
            theme={theme}
          />
      ...
      {page !== "landing" && user && (
        <WikiWikiBot user={user} t={t} theme={theme} />
      )}
    </div>
  );
}
